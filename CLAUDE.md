...
| `TLS_CERT` | `""` | TLS certificate path; empty = plain HTTP mode |
| `TLS_KEY` | `""` | TLS private key path; empty = plain HTTP mode |
...
- **quic-go** (HTTP/3) — dual-stack HTTP/3 (QUIC/UDP) + HTTP/2 (TCP) with Alt-Svc advertisement.
